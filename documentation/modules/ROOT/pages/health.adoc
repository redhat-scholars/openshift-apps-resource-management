...
[IMPORTANT]
====
The hellosalut API used to live at `https://fourtonfish.com/hellosalut/`. That address now answers
`301 Moved Permanently` towards `https://stefanbohacek.com/hellosalut/`. The MicroProfile REST Client
does not follow redirects by default, and the `UrlHealthCheck` you add below asserts a `200`, so the
old host silently turns the readiness probe red. Use `https://hellosalut.stefanbohacek.com` — it serves
the same JSON directly at `/`, which is why both `@Path` annotations above are `/`.
...
As the database readiness is already assessed, we can customize another readiness probe to check the availability of the endpoint `https://hellosalut.stefanbohacek.com`:
  ...
  "checks": [
    {
      ...
      "status": "UP",
      "data": {
        "host": "GET https://hellosalut.stefanbohacek.com/?lang=en"
      }
    },
    {
