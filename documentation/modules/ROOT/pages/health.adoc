...
----
          ...
          startupProbe: # <1>
            ...
            timeoutSeconds: 10
----
<1> Quarkus 3 also generates a `startupProbe`. It is what protects a slow-starting JVM from being
    killed by the liveness probe — but it gives the application only
    `initialDelaySeconds + failureThreshold × periodSeconds` ≈ 35 seconds to answer `/q/health/started`.
    Remember this when you set a CPU limit in xref:resources.adoc[Adjust resource quotas]: too small a
    limit makes the JVM miss that budget and the container ends up in `CrashLoopBackOff`.
...
[source,bash,subs="+macros,+attributes"]
----
./mvnw quarkus:add-extension -Dextensions="io.quarkus:quarkus-rest-client,io.quarkus:quarkus-rest-client-jsonb" # <1>
----
<1> `quarkus-rest-client` on its own has no JSON provider, so the `ExternalGreeting` response cannot be
    deserialized. Add `quarkus-rest-client-jsonb` to match the `quarkus-rest-jsonb` you are already using.
...
And add in `src/main/resources/application.properties`, using the greeting service URL your
instructor gave you:
...
Whatever the URL, it serves the same JSON directly at `/`, which is why both `@Path`
annotations above are `/`:
...
[NOTE]
====
That service is `apps/greeting-api` in this repository, deployed into the cluster by whoever
runs the workshop — see xref:greeting-api.adoc[Greeting API (instructor setup)]. Working
through the tutorial on your own? Deploy it yourself with the three commands in that chapter,
or fall back to the public API it stands in for:
...
Be aware of what you are taking on. `MessageInitializer` below calls this URL from a
`@Startup` hook, so if the host is unreachable the application does not start at all — you
get `Failed to start quarkus` and an `UnknownHostException`, not a red health check. A flaky
network or a busy conference Wi-Fi is enough.

Note also that the public API used to live at `https://fourtonfish.com/hellosalut/`. That
address now answers `301 Moved Permanently`, the MicroProfile REST Client does not follow
redirects by default, and the `UrlHealthCheck` you add below asserts a `200` — so the old
host silently turns the readiness probe red.
====
...
the three probes in `target/kubernetes/openshift.yml` now point at `/health/live`, `/health/ready` and `/health/started`.
...
As the database readiness is already assessed, we can customize another readiness probe to
check the availability of the greeting endpoint. Note that it reads the very same property
you set above, so the check follows whichever URL you configured:
...
public class CustomHealthCheck {
    ...
    HealthCheck checkURL() {
        return new UrlHealthCheck(externalURL+"/?lang=en") // <2>
...
Once the application runs, the readiness endpoint reports both the datasource check contributed by
`quarkus-agroal` and your own `external-url-check`:
  ...
  "checks": [
    {
      ...
      "status": "UP",
      "data": {
        "host": "GET https://greeting-api-<namespace>.<cluster-domain>/?lang=en"
      }
    },
    {
...
NOTE: `health-ui` is a dev-mode only feature. On the deployed application only `/health`,
`/health/live`, `/health/ready` and `/health/started` answer.