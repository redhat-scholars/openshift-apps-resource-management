...
----
mvn "io.quarkus.platform:quarkus-maven-plugin:3.39.4:create" \
...
Quarkus 3 publishes the plugin under the `io.quarkus.platform` group id. Pinning the version keeps
...
This creates a `tutorial-app` directory next to where you ran the command. Every command from
here to the end of the tutorial is run *from inside that directory*, so change into it now:
...
The generated project contains an endpoint `GreetingResource` served at `/messages`, along with
a `./mvnw` Maven wrapper — that wrapper is what the rest of the tutorial calls, and it only
exists inside `tutorial-app`. If `./mvnw` reports `No such file or directory`, you are in the
wrong directory.

You can customize the extensions that your project will use by running:
[.console-input]
[source,bash,subs="+macros,+attributes"]
----
